...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.if_cls_token:
            ...
            #     token_position = M // 2
            #     patch_num = int(math.sqrt(M))
            #     x = torch.cat((x[:, :token_position, :], cls_token[:, :patch_num], x[:, token_position:, :]), dim=1)
            #     x = x.reshape(B, patch_num + 1, patch_num, -1).permute(0, 2, 1, 3)
            #     x = x.reshape(B, -1, lat_dim)
            #     x = torch.cat((x[:, :token_position, :], cls_token[:, patch_num:], x[:, token_position:, :]), dim=1)
            #     x = x.reshape(B, patch_num + 1, patch_num + 1, -1).permute(0, 2, 1, 3).reshape(B, -1, lat_dim)
            #     token_position_final = x.shape[1] // 2
            if self.bimamba_type == '2d':
                pass
                # cls_token = self.cls_token.expand(B, -1, -1)
                # patch_num = int(math.sqrt(M))
                # x = torch.cat((x, cls_token[:, :patch_num]), dim=1)
                # x = x.reshape(B, patch_num + 1, patch_num, -1).permute(0, 2, 1, 3)
                # x = x.reshape(B, -1, lat_dim)
                # x = torch.cat((x, cls_token[:, patch_num:]), dim=1)
                # x = x.reshape(B, patch_num + 1, patch_num + 1, -1).permute(0, 2, 1, 3).reshape(B, -1, lat_dim)
                # token_position_final = -1
        ...
        if self.if_cls_token:
            if self.bimamba_type == '2d':
                # return hidden_states[:, token_position_final, :]
                pass
            elif self.use_double_cls_token:
...
@register_model
def vim_tiny_patch16_224_bimamba2d_final_pool_mean_abs_pos_embed_with_midclstok_div2_depth12(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=12, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="2d", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
